...
pub fn searchConfigAst(self: *Generator) !void {
    var output = try std.fs.cwd().createFile("src/build/generated.zig", .{});
    defer output.close();
...
pub fn searchActionsAst(self: *Generator) !void {
    const paths = ActionPath{ .version = "../cli/version.zig", .@"list-fonts" = "../cli/list_fonts.zig", .@"list-keybinds" = "../cli/list_keybinds.zig" };

    var output = try std.fs.cwd().openFile("src/build/generated.zig", .{ .mode = .read_write });
