...
pub const Trigger = struct {
    ...
    fn foldedCodepoint(cp: u21) [3]u21 {
        ...
        // Unicode slow path. Simple case folding always maps a single
        // codepoint to a single codepoint (full case folding can expand,
        // e.g. ß to "ss"), so no buffer is needed.
        const folded = uucode.get(.case_folding_simple, cp);
        return .{ folded, 0, 0 };
    }
