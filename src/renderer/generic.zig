...
pub fn Renderer(comptime GraphicsAPI: type) type {
    return struct {
        ...
        fn updateCustomShaderUniforms(self: *Self) !void {
            ...
            const current_cursor_visible: bool = self.cursor_state.visible;

            // cursor visibility changed
            if (@intFromBool(current_cursor_visible) != uniforms.current_cursor_visible) {
            ...
            if (current_cursor_visible) {
                ...
                // Get the current cursor glyph info
                self.cursor_state.glyph = self.cells.getCursorGlyph();

                //if we have a cursor in the fg cells.
                if (self.cursor_state.glyph) |cursor| {
                    ...
                    const current_cursor_style: i32 = @intFromEnum(self.cursor_state.style);
