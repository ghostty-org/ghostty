...
///
/// Because translation modifiers are stripped, a binding cannot require a
/// modifier that is also needed to type the character itself. On a US keyboard,
/// `{` is typed as `shift+[`, so a trigger `ctrl+shift+{` is impossible to
/// match. `ctrl+{` and `ctrl+shift+[` are proper.
///
