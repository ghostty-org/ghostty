...
pub const Surface = extern struct {
    ...
    /// Sync the broadcast group border overlay to match the given
    ...
    pub fn syncBroadcastGroup(
        ...
        // Rather than track which class we previously added, remove
        // every possible class: removing a class that isn't present
        // is not an error.
        var buf: [32]u8 = undefined;
        for (0..std.math.maxInt(u4) + 1) |old| {
            priv.broadcast_overlay.removeCssClass(
                broadcastColorClass(&buf, @intCast(old)),
            );
        }
        if (value.group) |v| {
            priv.broadcast_overlay.addCssClass(broadcastColorClass(&buf, v));
        }
        priv.broadcast_revealer.setRevealChild(@intFromBool(value.group != null));
    }

    /// The CSS class carrying the border color for the given broadcast
    /// group. Must match the class names generated by the application's
    /// runtime CSS.
    fn broadcastColorClass(buf: []u8, group: u4) [:0]const u8 {
        return std.fmt.bufPrintZ(buf, "broadcast-color-{d}", .{group}) catch
            unreachable;
    }
