...
pub fn init(self: *Surface, app: *App, opts: Options) !void {
    ...
    // Scroll events
    const ec_scroll = gtk.EventControllerScroll.new(.{ .vertical = true, .horizontal = true });
    ...
    const drop_target = gtk.DropTarget.new(gobject.ext.types.invalid, .{ .copy = true });
...
fn gtkDrop(
    ...
    switch (value.getGtype()) {
        gobject.ext.types.boxed => {
            ...
            const fl: *gdk.FileList = @ptrCast(@alignCast(boxed));
            var l_: ?*glib.SList = fl.getFiles();

            while (l_) |l| : (l_ = l.f_next) {
                const file: *gio.File = @ptrCast(@alignCast(l.f_data orelse continue));
