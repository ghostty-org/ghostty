...
extension Ghostty {
    ...
    class SurfaceView: OSView, ObservableObject, Codable, Identifiable {
        ...
        /// character range, used exclusively by accessibility overrides.
        ///
        /// This is separate from `cachedScreenContents` because computing
        /// viewport byte offsets requires a pin map (generated by
        /// `selectionString` on the Zig side), which is expensive and
        /// unnecessary for non-accessibility callers. The existing caches
        /// use the cheaper `ghostty_surface_read_text` path that produces
        /// plain text without positional metadata.
        ///
