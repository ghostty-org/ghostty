...
class TerminalController: BaseTerminalController, TabGroupCloseCoordinator.Controller {
    override var windowNibName: NSNib.Name? {
        // Ghostties workspace mode always uses the base Terminal nib. The
        // sidebar replaces native tabs and provides its own titlebar
        // appearance, so we bypass macosTitlebarStyle entirely to avoid
        // fighting complex titlebar subclasses (TitlebarTabsVenturaTerminalWindow,
        // TerminalTabsTitlebarTahoe, etc.).
        return "Terminal"
    }
    ...
    override func windowDidLoad() {
        ...
        // WorkspaceViewContainer internally hosts a TerminalViewContainer; we
        // forward upstream's initialContentSize hint to that inner container so
        // intrinsicContentSize is correct immediately (without waiting for
        // @FocusedValue propagation).
        let workspaceContainer = WorkspaceViewContainer(
            ghostty: self.ghostty,
            viewModel: self,
            delegate: self,
        )
        workspaceContainer.terminalContainer.initialContentSize = focusedSurface?.initialSize

        window.contentView = workspaceContainer
