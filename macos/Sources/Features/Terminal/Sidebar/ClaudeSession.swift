...
enum ClaudeSession {
    ...
    ///
    /// Submitted with a carriage return, which is the byte Enter actually
    /// sends. A line feed is not: line editors that treat injected text as
    /// a paste insert it literally, leaving the command sitting at the
    /// prompt unexecuted.
    @MainActor
